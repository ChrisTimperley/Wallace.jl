...
function individual_type(
  ...
  for stage in stages
    push!(definition, "$(stage.label)::Wallace.IndividualStage{$(chromosome(stage))}")
  end
  ...
  push!(definition, "constructor() = new(" * 
    join(vcat(["false"], ["Wallace.IndividualStage{$(chromosome(stages[i]))}()" for i in 1:length(stages)]), ",") *  
    ")"
  )

  # Build the full constructor.
  push!(definition, "constructor(" *
    join(["s$(i)::Wallace.IndividualStage{$(chromosome(stages[i]))}" for i in 1:length(stages)], ",") *
    ") = new(" *
  ...
  unshift!(definition, "type <: Wallace.Individual")
  push!(definition, "end")
  t = anonymous_type(Main, join(definition, "\n"))
